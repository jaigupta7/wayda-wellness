...
if page == "New Report":
    ...
    if st.session_state.extracted:
        ...
        for source_key, fields in st.session_state.extracted.items():
            with st.expander(f"📄 {source_key.upper()} report", expanded=False):
        ...
        if st.button("Confirm & Generate Final Report", type="primary"):
            ...
            if not template_path:
                st.error("Please upload the Final.pdf template below first, then click again.")
            else:
        ...
        st.divider()
        st.subheader("Template file (Final.pdf design)")
        template_file = st.file_uploader("Upload the Final.pdf template (static pages 2,3,10+ get reused from this)",
                                          type="pdf", key="template_upload")
        if template_file:
            tdir = tempfile.mkdtemp()
            tpath = os.path.join(tdir, "Final.pdf")
            with open(tpath, "wb") as out:
                out.write(template_file.getbuffer())
            st.session_state.template_path = tpath
